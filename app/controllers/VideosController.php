...
class VideosController extends \BaseController {
  ...
  public function store()
  {
    ...
    if (isset($collectionName)) {
      ...
      if (!$collectionId) {
        Session::flash('message', Lang::get('videos.controller.store.error'));
    ...
    if ($this->user->hasVideoFromHash($hash)) {
      if ($extension) {
        Session::flash('message', Lang::get('extension.store.alreadyadded'));
        return Redirect::route('extension.close')->with(array('message' => Lang::get('extension.store.alreadyadded')));
      } else {
        Session::flash('message', Lang::get('videos.controller.store.alreadyadded'));
    ...
    if ($exists) {
      ...
      if (!(bool)$created) {
        if ($extension) {
          Session::flash('message', Lang::get('extension.store.error'));
      ...
      try {
        ...
        if ($extension) {
          Session::flash('message', Lang::get('extension.store.alreadyprocessing'));
          return Redirect::route('extension.close')->with('message', Lang::get('extension.store.alreadyprocessing'));
        } else {
          Session::flash('message', Lang::get('videos.controller.store.alreadyprocessing'));
    ...
    if ($extension) {
      Session::flash('message', Lang::get('extension.store.success'));
      return Redirect::route('extension.close')->with('message', Lang::get('extension.store.success'));
    } else {
      Session::flash('message', Lang::get('videos.controller.store.success'));
  ...
  public function update()
  {
    ...
    if ($this->user->hasVideo($video->id)) {
      ...
      $saved = $video->save();
      if (!$saved) {
        Session::flash('message', Lang::get('videos.controller.update.error'));
        return Redirect::back()->with('message', Lang::get('videos.controller.update.error'));
      }
      Session::flash('message', Lang::get('videos.controller.update.success'));
  ...
  public function destroy($id)
  {
    ...
    if ($this->user->hasVideo($video->id)) {
      $video->delete();
      Session::flash('message', Lang::get('videos.controller.destroy.success'));
